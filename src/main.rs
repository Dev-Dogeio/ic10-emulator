...
use ic10_emulator::{
    CableNetwork, DaylightSensor, Device, ItemIntegratedCircuit10, LogicType,
    conversions::packed_number_to_text,
    devices::{AirConditioner, ICHostDevice, LogicMemory},
    parser::preprocess,
    types::shared,
};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let memory_aio = LogicMemory::new(None);
    let memory_hours = LogicMemory::new(None);
    let memory_minutes = LogicMemory::new(None);
    let memory_seconds = LogicMemory::new(None);
    ...
    j ra"#,
        memory_hours.borrow().get_id(),
        memory_minutes.borrow().get_id(),
        memory_seconds.borrow().get_id()
    );
    ...
    while !(chip.borrow().is_halted() || ac.borrow().read(LogicType::On)? == 0.0) {
        network.borrow().update(tick);
        let steps = ac.borrow().get_last_executed_instructions();
