...
mod tests {
    ...
    fn test_new_network_is_empty() {
        ...
        assert!(network.borrow().all_device_ids().is_empty());
    }
    ...
    fn test_add_single_device() {
        ...
        assert_eq!(network.borrow().all_device_ids(), vec![1]);
    }
    ...
    fn test_add_multiple_devices() {
        ...
        assert_eq!(network.borrow().all_device_ids(), vec![1, 2, 3, 4, 5]);
    }
    ...
    fn test_device_ref_id_ordering() {
        ...
        let all_ids = network.borrow().all_device_ids();
    ...
    fn test_remove_nonexistent_device() {
        let network = CableNetwork::new();
        let removed = network.borrow_mut().remove_device(999);
    ...
    fn test_remove_device_from_indices() {
        ...
        assert_eq!(network.borrow().get_devices_by_name(200).len(), 3);

        // Remove one device
        network.borrow_mut().remove_device(2);
    ...
    fn test_multiple_prefab_hashes() {
        ...
        assert_eq!(network.borrow().get_devices_by_prefab(300), vec![5]);
        assert!(network.borrow().get_devices_by_prefab(999).is_empty());
    }
    ...
    fn test_multiple_name_hashes() {
        ...
        assert_eq!(network.borrow().get_devices_by_name(3000), vec![5]);
        assert!(network.borrow().get_devices_by_name(9999).is_empty());
    }
    ...
    fn test_count_devices_by_prefab() {
        ...
        assert_eq!(network.borrow().count_devices_by_prefab(999), 0);
    }
    ...
    fn test_count_devices_by_name() {
        ...
        assert_eq!(network.borrow().count_devices_by_name(999), 0);
    }
    ...
    fn test_update_device_name() {
        ...
        assert!(network.borrow().get_devices_by_name(300).is_empty());

        // Update the name hash
        device.borrow().name_hash.set(300);
        network.borrow_mut().update_device_name(1, 200, 300);

        // Should now be in name index 300
        assert!(network.borrow().get_devices_by_name(200).is_empty());
        assert_eq!(network.borrow().get_devices_by_name(300), vec![1]);
    }
    // ==================== Batch Read Tests ====================
    ...
    fn test_batch_write_by_prefab() {
        ...
        for i in 1..=3 {
            ...
            let device = net.get_device(i).unwrap();
    ...
    fn test_device_ordering_maintained_after_removals() {
        ...
        // Remove device 3
        network.borrow_mut().remove_device(3);
